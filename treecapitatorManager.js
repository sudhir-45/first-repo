...
export class treecapitatorManager {
    ...
    static getBlocks(block, source, item, mainhand, durComp, unbreakingLevel) {
        ...
        function tick(block, takeDur) {
            ...
            if (blockNum >= treecapitatorManager.limit) return;
            if (stop == true) return;
            ...
            treecapitatorManager.break(block);
            if (takeDur == true && treecapitatorManager.reduceDurability(unbreakingLevel)) {
            ...
            for (const newblock of blocks){
                if (newblock != undefined) {
                    ...
                    if (newblock.typeId == sameID && !treecapitatorManager.isDenied(newloc, deniedLocs)) {
                        blockNum = blockNum + 1;
